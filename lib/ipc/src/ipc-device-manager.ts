...
export class IPCDeviceManager {
  ...
  public addResource(ipc: IPCHost, device: ResourceDevice) {
    ...
    ipc.handle<void, string[]>("find-libraries", async () =>
      (await device.findLibraries()).map((path) =>
    ...
    ipc.handle<{ multiple: boolean }, string[] | undefined>(
      "browse-libraries",
      async ({ multiple }) =>
        (await device.browseLibraries(multiple))?.map((path) =>
          pathToFileURL(path).toString(),
        ),
    );
  }
  ...
  public async invokeAgent(obj: {
    ...
    agent.on("session-event", (event) => {
      // Forward all session events to editor as notification except permission
      // requests since their resolution are handled here.
      if (event.type !== "permission-request") {
        ipc.notify("agent-session-event", { conversationId, event });
      }

      switch (event.type) {
        case "tool-use":
          if (event.name === "mcp__pencil__batch_design" && event.streaming) {
            ipc.request("batch-design", {
              conversationId,
              filePath: event.input?.filePath,
              operations: event.input?.operations,
              id: event.id,
              partial: true,
            });
          } else if (
            event.name === "mcp__pencil__spawn_agents" &&
            event.streaming
          ) {
            ipc.request("spawn-agents", {
              conversationId,
              filePath: event.input?.filePath,
              agentsConfig: event.input?.agentsConfig,
              id: event.id,
              partial: true,
            });
          }
